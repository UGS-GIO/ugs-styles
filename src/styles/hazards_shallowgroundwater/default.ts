/**
 * Shallow Groundwater Susceptibility — polygons colored by `sgshazardunit` (the SLD
 * classification field for this layer), with a uniform black stroke (SLD default). Ported from
 * the retiring GeoServer SLD hazards:shallowgroundwater: 3 `PropertyIsEqualTo` rules, each a
 ...
 * `hazards_shallowgroundwater`, matching every sibling hazards item in the live catalog (e.g.
...
const FIELD = 'sgshazardunit';

// sgshazardunit code -> exact fill hex, from shallowgroundwater.sld (rule order kept).
const SHALLOW_GW_FILL: Record<string, string> = {
    'S1sgs': '#002673',     // Shallow Groundwater Unit 1
    'S2sgs': '#0070ff',     // Shallow Groundwater Unit 2
    'S3sgs': '#73b2ff',     // Shallow Groundwater Unit 3
};

// Human labels for the legend — the SLD rule Titles, keyed by code so a swatch matches its fill.
const SHALLOW_GW_LABELS: Record<string, string> = {
    'S1sgs': 'Shallow Groundwater Unit 1',
    'S2sgs': 'Shallow Groundwater Unit 2',
    'S3sgs': 'Shallow Groundwater Unit 3',
};

// Unmapped/blank codes -> grey (the categorical archetype's default fallback). Not a real data class.
const OTHER = '#BDBDBD';

const fillColor = matchByField(FIELD, SHALLOW_GW_FILL, OTHER);
const strokeColor = '#000000';

export const spec = {
    itemId: 'hazards_shallowgroundwater',
    render: 'default',
    kind: 'vector',
    assets: ['pmtiles'],
    ...
    legend: [
        ...Object.entries(SHALLOW_GW_FILL).map(([code, color]) => ({
            label: SHALLOW_GW_LABELS[code] ?? code,
            color,
        })),
