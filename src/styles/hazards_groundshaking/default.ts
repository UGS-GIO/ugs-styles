/**
 * Groundshaking Hazard — polygons colored by `egshazardunit` (the SLD classification field for
 * this layer), with a uniform black stroke (SLD default). Ported from the retiring GeoServer SLD
 * hazards:groundshaking: 5 `PropertyIsEqualTo` rules, each a solid fill + stroke, no else-rule.
 ...
 * `hazards_groundshaking`, matching every sibling hazards item in the live catalog (e.g.
...
const FIELD = 'egshazardunit';

// egshazardunit code -> exact fill hex, from groundshaking.sld (rule order kept).
const GROUNDSHAKING_FILL: Record<string, string> = {
    'LMegs': '#73dfff',     // Light/Moderate
    'MSegs': '#70a800',     // Moderate/Strong
    'SVSegs': '#e6e600',    // Strong/Very Strong
    'VSSegs': '#e69800',    // Very Strong/Severe
    'SVegs': '#e60000',     // Severe/Violent
};

// Human labels for the legend — the SLD rule Titles, keyed by code so a swatch matches its fill.
const GROUNDSHAKING_LABELS: Record<string, string> = {
    'LMegs': 'Light/Moderate',
    'MSegs': 'Moderate/Strong',
    'SVSegs': 'Strong/Very Strong',
    'VSSegs': 'Very Strong/Severe',
    'SVegs': 'Severe/Violent',
};

// Unmapped/blank codes -> grey (the categorical archetype's default fallback). Not a real data class.
const OTHER = '#BDBDBD';

const fillColor = matchByField(FIELD, GROUNDSHAKING_FILL, OTHER);
const strokeColor = '#000000';

export const spec = {
    itemId: 'hazards_groundshaking',
    render: 'default',
    kind: 'vector',
    assets: ['pmtiles'],
    title: 'Groundshaking Hazard',
    ...
    legend: [
        ...Object.entries(GROUNDSHAKING_FILL).map(([code, color]) => ({
            label: GROUNDSHAKING_LABELS[code] ?? code,
            color,
        })),
