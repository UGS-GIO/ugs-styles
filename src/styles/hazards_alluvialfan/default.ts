/**
 * Statewide Active Alluvial-Fan Inventory — polygons colored by `aafhazardunit` (the SLD
 * classification field for this layer), with a uniform grey stroke (#6e6e6e). Ported from the
 * retiring GeoServer SLD hazards:alluvialfan: 3 `PropertyIsEqualTo` rules, each a solid fill +
 ...
 * `hazards_alluvialfan`, matching every sibling hazards item in the live catalog (e.g.
...
const FIELD = 'aafhazardunit';

// aafhazardunit code -> exact fill hex, from alluvialfan.sld (rule order kept).
const ALLUVIAL_FAN_FILL: Record<string, string> = {
    'Haaf': '#ff0000',      // High Priority Alluvial Fan
    'Maaf': '#0070ff',      // Moderate Priority Alluvial Fan
    'Oaaf': '#ffffbe',      // Out of domain- Arizona
};

// Human labels for the legend — the SLD rule Titles, keyed by code so a swatch matches its fill.
const ALLUVIAL_FAN_LABELS: Record<string, string> = {
    'Haaf': 'High Priority Alluvial Fan',
    'Maaf': 'Moderate Priority Alluvial Fan',
    'Oaaf': 'Out of domain- Arizona',
};

// Unmapped/blank codes -> grey (the categorical archetype's default fallback). Not a real data class.
const OTHER = '#BDBDBD';

const fillColor = matchByField(FIELD, ALLUVIAL_FAN_FILL, OTHER);
const strokeColor = '#6e6e6e';

export const spec = {
    itemId: 'hazards_alluvialfan',
    render: 'default',
    kind: 'vector',
    assets: ['pmtiles'],
    ...
    legend: [
        ...Object.entries(ALLUVIAL_FAN_FILL).map(([code, color]) => ({
            label: ALLUVIAL_FAN_LABELS[code] ?? code,
            color,
        })),
...
const layers: StyleLayer[] = [
    {
        id: 'hazards_alluvialfan-fill',
        type: 'fill',
        // SLD fill-opacity was 1.0 (solid) on every rule.
        paint: { 'fill-color': fillColor, 'fill-opacity': 1 },
    },
    {
        id: 'hazards_alluvialfan-line',
        type: 'line',
        paint: { 'line-color': strokeColor, 'line-width': 0.9 },
    },
];
