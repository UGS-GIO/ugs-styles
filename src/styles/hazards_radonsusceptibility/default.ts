/**
 * Radon Susceptibility — polygons colored by `grshazardunit` (the SLD classification field for
 * this layer), with a uniform black stroke (SLD default). Ported from the retiring GeoServer SLD
 * hazards:radonsusceptibility: 3 `PropertyIsEqualTo` rules, each a solid fill + stroke, no
 ...
 * `hazards_radonsusceptibility`, matching every sibling hazards item in the live catalog (e.g.
...
const FIELD = 'grshazardunit';

// grshazardunit code -> exact fill hex, from radonsusceptibility.sld (rule order kept).
const RADON_FILL: Record<string, string> = {
    'Hgrs': '#ff0000',      // High Hazard
    'Mgrs': '#ffaa00',      // Moderate Hazard
    'Lgrs': '#38a800',      // Low Hazard
};
...
const fillColor = matchByField(FIELD, RADON_FILL, OTHER);
const strokeColor = '#000000';

export const spec = {
    itemId: 'hazards_radonsusceptibility',
    render: 'default',
    kind: 'vector',
    assets: ['pmtiles'],
    ...
    legend: [
        ...Object.entries(RADON_FILL).map(([code, color]) => ({
            label: RADON_LABELS[code] ?? code,
            color,
        })),
