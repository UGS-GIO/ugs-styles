/**
 * Legacy Landslide Compilation — polygons colored by `lschazardunit` (the SLD classification
 * field for this layer), with a uniform black stroke (SLD default). Ported from the retiring
 * GeoServer SLD hazards:landslidelegacy: 1 `PropertyIsEqualTo` rules, each a solid fill +
 ...
 *
 * Stroke color: none of the 1 rules set a CssParameter name="stroke" (only stroke-width), so
 ...
 * `hazards_landslidelegacy`, matching every sibling hazards item in the live catalog (e.g.
...
const FIELD = 'lschazardunit';

// lschazardunit code -> exact fill hex, from landslidelegacy.sld (rule order kept).
const LANDSLIDE_LEGACY_FILL: Record<string, string> = {
    'Ulsc': '#ffa77f',      // Mapped Legacy Landslide
};

// Human labels for the legend — the SLD rule Titles, keyed by code so a swatch matches its fill.
const LANDSLIDE_LEGACY_LABELS: Record<string, string> = {
    'Ulsc': 'Mapped Legacy Landslide',
};

// Unmapped/blank codes -> grey (the categorical archetype's default fallback). Not a real data class.
const OTHER = '#BDBDBD';

const fillColor = matchByField(FIELD, LANDSLIDE_LEGACY_FILL, OTHER);
const strokeColor = '#000000';

export const spec = {
    itemId: 'hazards_landslidelegacy',
    render: 'default',
    kind: 'vector',
    assets: ['pmtiles'],
    ...
    legend: [
        ...Object.entries(LANDSLIDE_LEGACY_FILL).map(([code, color]) => ({
            label: LANDSLIDE_LEGACY_LABELS[code] ?? code,
            color,
        })),
